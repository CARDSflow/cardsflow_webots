...
def basicPhysics(proto):
    ...
    proto.write('      mass 1\n')
    proto.write('      inertiaMatrix [1 1 1 0 0 0]\n')
    proto.write('      centerOfMass [0 0 0]\n')
    proto.write('    }\n')

def URDFLink(proto, link, level, parentList, childList, linkList, jointList, jointPosition = [0.0, 0.0, 0.0], jointRotation = [1.0, 0.0, 0.0, 0.0]):
    indent = '  '
    haveChild = 0
...
def URDFJoint(proto, joint, level, parentList, childList, linkList, jointList):
    ...
    if joint.type == 'revolute' or joint.type == 'continuous':
        ...
        for childLink in linkList:
            if childLink.name == joint.child:
                URDFLink(proto, childLink, level, parentList, childList, linkList, jointList, joint.position, joint.rotation)
        return
    ...
    for childLink in linkList:
        if childLink.name == joint.child:
            URDFLink(proto, childLink, level + 1, parentList, childList, linkList, jointList, joint.position, joint.rotation)
    proto.write(level * indent + '}\n')

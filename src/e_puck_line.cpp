...
cardsflow_webots::set_int setTimeStepSrv;
...
int main(int argc, char **argv) {
  ...
  setTimeStepClient = n.serviceClient<cardsflow_webots::set_int>(modelName + "/robot/time_step");
  setTimeStepSrv.request.value = step;

  std::vector<ros::ServiceClient> enableDistSensorClient;
  cardsflow_webots::set_int enableDistSensorSrv;
  ...
  for (i = 0; i < NB_DIST_SENS; i++) {
    ...
    enableDistSensorClient.push_back(n.serviceClient<cardsflow_webots::set_int>(modelName + '/' + deviceName + "/enable"));
  ...
  cardsflow_webots::set_float wheelSrv;
  wheelSrv.request.value = INFINITY;
  ros::ServiceClient leftWheelPositionClient =
    n.serviceClient<cardsflow_webots::set_float>(modelName + "/left_wheel_motor/set_position");
  leftWheelPositionClient.call(wheelSrv);
  ros::ServiceClient rightWheelPositionClient =
    n.serviceClient<cardsflow_webots::set_float>(modelName + "/right_wheel_motor/set_position");
  rightWheelPositionClient.call(wheelSrv);
  ros::ServiceClient leftWheelVelocityClient =
    n.serviceClient<cardsflow_webots::set_float>(modelName + "/left_wheel_motor/set_velocity");
  ros::ServiceClient rightWheelVelocityClient =
    n.serviceClient<cardsflow_webots::set_float>(modelName + "/right_wheel_motor/set_velocity");

  // turn the leds on
  std::vector<ros::ServiceClient> setLedClient;
  cardsflow_webots::set_int setLedSrv;
  sprintf(deviceName, "led0");
  setLedClient.push_back(n.serviceClient<cardsflow_webots::set_int>(modelName + '/' + deviceName + "/set_led"));
